...
final class AudioDucker: @unchecked Sendable {
    ...
    private func performDuck() {
        ...
        if originalVolume != nil, let deviceID = duckedDeviceID {
            ...
            // in-between value.
            writeDuckTarget(deviceID: deviceID)
            return
        }
        ...
        guard current > configuration.targetVolume else {
            // §3.1: already quiet enough — leave `originalVolume` nil so restore() no-ops too.
            owLog("[AudioDucker] duck() skipped — already at/below target (\(current) <= \(configuration.targetVolume))")
            return
        }
    ...
    private func writeDuckTarget(deviceID: AudioDeviceID) {
        let target = configuration.targetVolume
    ...
    private func performRestoreStart() {
        ...
        let duckedLevel = max(configuration.targetVolume, 0.0001) // floor avoids a zero-ratio blowup
